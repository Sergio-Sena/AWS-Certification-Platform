// CLF-C02 Script Simplificado - Versão sem erros
...
function showAssessmentQuestion() {
    if (!window.clfInitialAssessment) {
        console.error('Avaliação inicial não carregada');
        return;
    }
    
    const question = window.clfInitialAssessment[assessmentState.currentQuestion];
    if (!question) return;
    ...
    const progress = ((assessmentState.currentQuestion + 1) / window.clfInitialAssessment.length) * 100;
...
function selectAssessmentOption(optionIndex) {
    if (!window.clfInitialAssessment) return;
    
    const questionId = window.clfInitialAssessment[assessmentState.currentQuestion].id;
...
function nextAssessmentQuestion() {
    ...
    if (assessmentState.currentQuestion < window.clfInitialAssessment.length - 1) {
...
function finishAssessment() {
    if (!window.assessCLFLevel) {
        console.error('Função de avaliação não encontrada');
        return;
    }
    
    const results = window.assessCLFLevel(assessmentState.answers);
...
function showTopic(topic) {
    if (!window.clfStudyMaterial) {
        console.error('Material de estudo não carregado');
        return;
    }
    
    const topicContent = document.getElementById('topic-content');
    const material = window.clfStudyMaterial[topic];
